...
import '../../styles/TopRatedMovie.css'
import number from "./TopMoviePage.json"
...
// { 
// * Images url for API fecth. In API I have string like:  /somethings  
// * and in next line I have url to get images.I add string then am fetching API data.->
// ! Movie_Image + apidata.image
// }    
    const Movie_Image = 'https://image.tmdb.org/t/p/w300/'

    useEffect(() =>{
        const url = `https://api.themoviedb.org/3/movie/top_rated?page=${moviePageIndex}`;
        const options = {
            method: 'GET',
            headers: {
  ...
  return (
    <div>
    <h1>Top <span className='popular_Movie_list_Title_span1'> Rared </span> On <span className='popular_Movie_list_Title_span1'> DB!</span> </h1>
    <div className="Top_rated_conteiner_flex">
        <div className="Top_rated_conteier"> 
          {apiData.results? (
            apiData.results.map(movie=>(
              <div  key={movie.id} className="Top_rated_conteier_box"> 
                <Link to={`${routes.top_rated}/${movie.id}`}>
                    <img src={`${Movie_Image}${movie.poster_path}`} alt={movie.title}  />
                    <div className="Top_rated_conteier_box_body">
                ...
                <h2 className="Top_rated_title">{movie.title} </h2>
              </div>
            ))
          ) :null }
        </div>
      ...
      <div className="display_f_center">
        <div className="Top_page_index">
        {number.map(number=>{
          return <input key={number.number} type="button" value={number.number} onClick={()=>{setMoviePageIndex(number.number),  window.scrollTo(0,0)}} />
        })}
      </div>
      </div>
    </div>
